...
export async function getSupabaseData<T>(key: string, defaultValue: T): Promise<T> {
  try {
    ...
    if (error) {
      ...
      if (localData) {
        try {
          ...
          console.warn(`[localStorage] Parse error for ${key}:`, parseError);
        }
      }
      // Initialize with default value in Supabase and return default
      ...
      try {
        ...
        if (localData) {
          try {
            ...
            console.warn(`[localStorage] Parse error for ${key}:`, localParseError);
          }
        }
    ...
    // Initialize with default value and return it
    try {
      await setSupabaseData(key, defaultValue);
    } catch (setError) {
      console.warn(`[Supabase] Could not save default value for ${key}:`, setError);
    }
    return defaultValue;
  }
}
